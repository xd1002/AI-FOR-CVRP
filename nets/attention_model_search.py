...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.n_agent = opts.n_agent

        # Problem specific context parameters (placeholder and step context dimension)
        if self.is_vrp or self.is_orienteering or self.is_pctsp:
            # Embedding of last node + remaining_capacity / remaining length / remaining prize to collect
            step_context_dim = opts.embedding_dim + 1
            ...
            if self.is_vrp and self.allow_partial:  # Need to include the demand if split delivery allowed
                self.project_node_step = nn.Linear(1, 3 * opts.embedding_dim, bias=False)
        else:  # TSP
            assert problem.NAME == "tsp", "Unsupported problem: {}".format(problem.NAME)
            step_context_dim = 2 * opts.embedding_dim  # Embedding of first and last node
            node_dim = 2  # x, y
            
            # Learned input symbols for first action
            self.W_placeholder = nn.Parameter(torch.Tensor(2 * opts.embedding_dim))
            self.W_placeholder.data.uniform_(-1, 1)  # Placeholder should be in range of activations

        self.init_embed = nn.Linear(node_dim, opts.embedding_dim)
        ...
        self.project_node_embeddings = nn.Linear(opts.embedding_dim, 3 * opts.embedding_dim, bias=False)
        self.project_fixed_context = nn.Linear(opts.embedding_dim, opts.embedding_dim, bias=False)
        self.project_step_context = nn.Linear(step_context_dim, opts.embedding_dim, bias=False)
        assert opts.embedding_dim % opts.n_heads == 0
        # Note n_heads * val_dim == embedding_dim so input to project_out is embedding_dim
        self.project_out = nn.Linear(opts.embedding_dim, opts.embedding_dim, bias=False)
