...
def generate_vrp_data(dataset_size, vrp_size, n_agent, n_depot):
    CAPACITIES = {
        10: 20.,
        30: 3000.,
        50: 40.,
    ...
    return list(zip(
        np.random.uniform(size=(dataset_size, n_depot, 3)).tolist(),  # Depot location
        np.random.uniform(size=(dataset_size, vrp_size+n_agent, 3)).tolist(),  # Node locations
        np.zeros((dataset_size, vrp_size+n_agent)).tolist(),  # Demand, uniform integer 1 ... 9
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_agent', type=int, default=3, help="number of UAV")
    parser.add_argument('--n_depot', type=int, default=3, help="number of depot")
    ...
    # opts.problem == 'vrp', opts.data_distribution == 'all'
    # {'vrp': [None]}
    else:
        problems = {
            opts.problem:
    ...
    # problem:'vrp', distributions:None
            ...
            for graph_size in opts.graph_sizes:
                ...
                if problem == 'tsp':
                    ...
                    dataset = generate_vrp_data(
                        opts.dataset_size, graph_size, opts.n_agent, opts.n_depot)
                elif problem == 'pctsp':
